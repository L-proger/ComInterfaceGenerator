...
public:
    CppGenerator(const std::string& outputPath) : Generator(outputPath){
        ...
        auto cstdintModule = TypeCache::addModule<Module>("cstdint");
        ...
        auto languagePrimitiveModule = TypeCache::addModule<Module>("LanguagePrimitive");
        ...
        auto stringModule = TypeCache::addModule<Module>("string");
        TypeCache::replaceType(primitiveModule->findType("string"), stringModule->addNewType("std::string"));


        auto comCoreModule = TypeCache::addModule<Module>("LFramework/COM/ComObject.h");
