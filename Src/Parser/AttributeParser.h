            ...
            for(auto& argCtx : ctx->argument_list()->argument()){
                ...
                if(expr != nullptr){
                    if(expr->STRING_LITERAL() != nullptr){
                        result->arguments.push_back(std::make_shared<StringConstant>(TypeCache::findPrimitiveType("string"), expr->getText()));
                    }else{
                        auto numeric = expr->numeric_literal();
                        if(numeric->float_literal() != nullptr){
                            if(numeric->float_literal()->FLOAT_SUFFIX() != nullptr){
                                result->arguments.push_back(std::make_shared<FloatConstant>(TypeCache::findPrimitiveType("float"), expr->getText()));
                            }else{
                                result->arguments.push_back(std::make_shared<DoubleConstant>(TypeCache::findPrimitiveType("double"), expr->getText()));
                            }
